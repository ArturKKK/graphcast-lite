...
def test_calm_with_missing_direction_is_zero_not_nan():
    ...
    В ISD-Lite при штиле направление помечено пропуском — его попросту нет.
    Прямая формула давала NaN, и все штили выпадали из оценки ветра: до
    28.08.2026 из 618 236 проверочных строк отбрасывалось 132 348, и часть их
    была именно штилями. Это как раз случаи с наибольшей относительной ошибкой
    модели, терять их нельзя.
    """
