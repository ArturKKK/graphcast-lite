...
Этот блок не входит в подаваемый текст. Он существует, чтобы можно было отдать
рукопись другой нейросети и получить придирчивый разбор. Скрипт сборки
отрезает всё, начиная с «## Перечень рисунков», так что в PDF ничего из этого
не попадает — но перед подачей всё равно проверь глазами.

Как пользоваться: скопировать текст ниже вместе с полной рукописью и списком
литературы в чистый диалог.
