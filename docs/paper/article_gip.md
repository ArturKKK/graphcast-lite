...
Точность оценок характеризуется доверительными интервалами, полученными блочным бутстрепом: поскольку сроки начала прогноза следуют через 6 ч и коррелированы внутри одного синоптического процесса, ресэмплированию подвергаются блоки подряд идущих сроков длиной в несколько суток, а не отдельные сроки [13]. Интервалы строятся из посрочных вкладов в ошибку, сохраняемых при каждом прогоне, поэтому парное сравнение двух конфигураций ведётся на общих сроках и не требует повторного счёта. Оговорим, что интервал учитывает изменчивость по срокам верификации, но не по повторным обучениям; последняя оценена отдельно и приведена в «Ограничениях».
...
Объём выборки в таблицах различается. На полной тестовой выборке из 1607 сроков (25.11.2019 — 30.12.2020, все четыре сезона) посчитаны табл. 1, 3 и 8. Расчёты, требующие длинной развёртки или внешних данных, выполнены на подвыборках из **первых** сроков тестового периода: 100 (по 19.12.2019) в табл. 2 и 5, 200 (по 13.01.2020) в табл. 8, 500 (по 28.03.2020) при оценке на пяти сутках. Они представляют холодное полугодие и систематически труднее полной выборки: ошибка приземной температуры на суточном сроке равна 1,46 °C на 1607 сроках, 1,72 °C на 200 и 2,05 °C на 100. Числа из разных таблиц между собой несопоставимы; внутри таблицы все конфигурации посчитаны на одних сроках.
...
У веса вставки обнаруживается внутренний максимум по агрегату. Рост доли вставки с 1,7 до 34,1 % улучшает и агрегат, и приземную температуру, тогда как дальнейший рост до 63,3 % агрегат ухудшает. Мера утверждения такова. Парный блочный бутстреп по общим срокам даёт для разности агрегатов при $W = 100$ и $W = 30$ величину −0,05 п.п. с интервалом [−0,07; −0,03], не накрывающим ноль: на данной тестовой выборке падение агрегата воспроизводимо. Оговорим границу этого вывода: интервал учитывает изменчивость по срокам, но не по повторным обучениям, а межзапусковая изменчивость оценена нами в 0,03 п.п. и сравнима с самой разностью. По приземной температуре максимума нет вовсе, она убывает монотонно (1,503 → 1,445 → 1,415 → 1,407). Внутренний максимум по агрегату, таким образом, установлен как свойство данной выборки, но не как свойство метода; объясняющий его механизм виден в последнем столбце увереннее. Механизм виден в последнем столбце: платой служит точность на глобальной части графа: сама ошибка при $W = 100$ выше, чем при $W = 30$, лишь на 0,8 % (0,1717 против 0,1703), но её ПРИРАЩЕНИЕ относительно конфигурации без весов втрое больше — 0,0021 против 0,0007. Прогноз внутри вставки за сутки существенно зависит от притока извне, поэтому деградация окружающего поля возвращается в область через развёртку. Оптимум, таким образом, определяется не самой областью, а балансом между ней и её окружением.

Веса каналов действуют как регулятор, а не как исправление ошибки: агрегатная успешность при их введении снижается на 0,30 п.п. (парный бутстреп, интервал [−0,32; −0,28]), тогда как приземная температура улучшается на 4,6 % (от 0,056 °C на +6 ч до 0,071 °C на +24 ч, все четыре интервала не накрывают ноль). Причина в том, что агрегат (3) сам представляет собой среднее по каналам с равными весами, то есть равномерное взвешивание для него оптимально по построению. Выравнивание вкладов смещает точность к каналам с малой собственной изменчивостью — приземной температуре, давлению, геопотенциалу — за счёт каналов с большой. В пределах измеренного диапазона выигрыш канала описывается логарифмом полученного им веса (коэффициент корреляции 0,98 по 12 каналам), однако за его пределы эта зависимость не переносится: усиление пяти приземных каналов вчетверо сверх выравнивания дало меньше предсказанного, а геопотенциал на 500 гПа при возросшем весе стал хуже. Механизм тот же, что и у веса вставки: усиленные каналы отбирают ресурс у полей, которые их же и определяют.
...
Эта линия использует прямое предсказание поля, а не остаточную формулировку, и потому её агрегатная успешность (около 60 %) существенно ниже, чем у 19-канальной строки табл. 1 (69,4 %), посчитанной на остаточной модели, — при близких значениях приземной температуры. Числа табл. 1 и табл. 4 относятся, таким образом, к разным моделям и между собой не сравниваются; внутри табл. 4 все конфигурации сравнимы.
...
**Табл. 6.** Подбор гиперпараметров оптимальной интерполяции на проверочной выборке (200 сроков, регион): агрегатная успешность и ошибка приземной температуры по горизонтам +6 / +12 / +18 / +24 ч.
...
**Табл. 7.** Усвоение наблюдений на тестовой выборке (1607 сроков, все сезоны, регион — 2501 узел). Ошибка приземной температуры, °C, по горизонтам и агрегатная успешность $\bar{S}$. Метрика вычислена по полю до коррекции (см. п. 5.1). Оптимальная интерполяция при 10 % — среднее по трём независимым расстановкам условных станций.
...
Из табл. 7 следуют четыре наблюдения.
...
Мотивировка усвоения опирается на деградацию прогноза при длительной развёртке, тогда как приведённые выше оценки ограничены сроком +24 ч. Поэтому выполнена серия расчётов с развёрткой на 56 шагов (336 ч) в четырёх режимах: без усвоения; с оптимальной интерполяцией на каждом шаге при плотности 10 %; с усвоением только на первых четырёх шагах; с усвоением на каждом шаге при плотности 1 %. Оценка — на 200 сроках тестовой выборки, область — региональная вставка (табл. 8).

**Табл. 8.** Ошибка прогноза приземной температуры (°C) при развёртке до 336 ч (14 суток). Основная конфигурация, 200 сроков, регион (2501 узел).
