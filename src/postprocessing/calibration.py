...
def spread_scale(sigma, err) -> float:
    """Во сколько раз домножить заявленный разброс, чтобы он стал честным.

    Возвращает отношение действительной среднеквадратичной ошибки к
    среднеквадратичному заявленному разбросу. Больше единицы — модель
    самоуверенна и разброс надо расширить.

    Одно число исправляет положение только если модель ПРАВИЛЬНО РАНЖИРУЕТ
    неуверенность, то есть отношение примерно одинаково во всех корзинах
    надёжности. У нашей вероятностной настройки так и вышло: 1,14-1,20 по всем
    десяти корзинам при общем 1,18. Если бы отношение гуляло, множитель чинил бы
    среднее и портил края.

    Настраивать множитель надо на ОДНОМ наборе, а применять к ДРУГОМУ: подогнав
    и проверив на одних данных, получишь единицу по построению и ничего не
    узнаешь.
    """
    sigma = np.asarray(sigma, dtype=np.float64)
    err = np.asarray(err, dtype=np.float64)
    denom = np.sqrt((sigma ** 2).mean())
    if denom <= 0:
        return 1.0
    return float(np.sqrt((err ** 2).mean()) / denom)
