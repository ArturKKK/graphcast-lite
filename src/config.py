...
class DataConfig(BaseModel):
    ...
    time_stride: int = 1
    # Шаг ВХОДА (между входными кадрами). 0 = как time_stride, то есть прежнее
    # поведение. Ставить меньше time_stride имеет смысл только для ПРЯМОГО
    # прогноза, где модель применяется один раз и авторегрессии нет.
    obs_stride: int = 0
...
class ExperimentConfig(BaseModel):
    ...
    # Сколько горизонтов мерить на валидации. 1 — как было всегда (только +6 ч).
    # Больше — отбор лучшего состояния и ранняя остановка считаются по среднему
    # по горизонтам, что честнее: одношаговая ошибка систематически промахивается
    # мимо дальних сроков (проверено четырежды за август 2026).
    val_ar_steps: int = 1
    ...
    # Отцеплять состояние между шагами AR-развёртки и делать градиент
    # пошагово. Память перестаёт расти с длиной развёртки: при полном
    ...
    # Три стандартных приёма, которых до 23.09.2026 в основной модели не было.
    # Все по умолчанию выключены — старые конфиги дают прежний результат.
    # Обрезка нормы градиента: при батче 1 градиент шумный, единичный выброс
    # уводит веса. 0 — выключено; у GraphCast-подобных моделей обычно 1.0.
    grad_clip_norm: float = 0.0
    # Накопление градиента: шаг оптимизатора раз в N примеров. Эффективный батч
    # растёт без роста памяти. Планировщик темпа считает шаги оптимизатора.
    grad_accum_steps: int = 1
    # Экспоненциальное среднее весов. 0 — выключено. Валидация, лучшая модель и
    # ema_last.pth идут по усреднённым весам; checkpoint.pth хранит сырые плюс EMA.
    ema_decay: float = 0.0
