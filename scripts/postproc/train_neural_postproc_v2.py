#!/usr/bin/env python
"""CLI для обучения v2 neural postprocessor (StationLeadAwareResidualMLP).

Отличия от v1:
  • station embedding (16d, learnable per ISD station);
  • lead-FiLM: модуляция первого слоя как функция lead_norm;
  • drop dead static features (urban_flag, dist_to_coast — константы 0 в корпусе);
  • feature_cols включают исправленные dewpoint_depression и реальные lsm/z_surf
    (см. scripts/postproc/build_corpus.py v2).

Пример:
    python scripts/postproc/train_neural_postproc_v2.py \\
        --train-parquet data/postproc/corpus_v2_train.parquet \\
        --val-parquet   data/postproc/corpus_v2_val.parquet \\
        --out-dir       experiments/neural_postproc_v2 \\
        --epochs 30 --batch-size 4096 --station-emb-dim 16
"""
...
# v2 feature set: drop urban_flag, dist_to_coast (dead constants in corpus_v2)
...
class TrainConfigV2:
    ...
    val_parquet: str
    out_dir: str
    feature_cols: List[str] = field(default_factory=lambda: list(DEFAULT_FEATURES_V2))
...
def _build_station_idx(train_parquet: str, val_parquet: str) -> Dict[str, int]:
    """Stable station→idx mapping covering both splits."""
    seen: set[str] = set()
...
def train_v2(cfg: TrainConfigV2) -> Dict[str, float]:
    _seed_all(cfg.seed)
    ...
    if train_ds.station_idx_arr is None or train_ds.lead_norm_arr is None:
        raise RuntimeError("v2 requires station_usaf + lead_norm in parquet")
...
def _parse_args() -> TrainConfigV2:
    p = argparse.ArgumentParser(description="Train v2 neural postprocessor (station+lead aware)")
    ...
    p.add_argument("--station-emb-dim", type=int, default=16)
    ...
    return TrainConfigV2(**cfg_kwargs)
