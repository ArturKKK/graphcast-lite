...
#
# Серия задаётся аргументами вида «обучающих:жребий». Придержано ВСЕГДА 14
# станций, и всегда одних и тех же при данном жребии — меняется только размер
# обучающей выборки.
#
# Так сделано после разбора 29.08.2026: сначала я менял число ПРИДЕРЖАННЫХ
# станций, а это меняло заодно состав проверки. Сырая ошибка проверочных наборов
# гуляла от 2,54 до 3,27 — совершенно разные совокупности, и кривая вышла
# немонотонной: при 64 обучающих станциях перенос оказался хуже, чем при 57.
# Постановка была неразрешимой, а не результат странным.
#
...
SERIES=("$@")
[[ ${#SERIES[@]} -eq 0 ]] && SERIES=(57:42 43:42 29:42 15:42 57:43 43:43 29:43 15:43)
HOLDOUT=14
...
run_one() {   # сколько станций на обучение, жребий
  local keep=$1 seed=$2 tag="k${1}s${2}"
  log ""
  log "########## обучающих станций $keep, придержано $HOLDOUT, жребий $seed ##########"
  ...
  if [[ ! -f "$DIR/${tag}_seen_test.parquet" ]]; then
    ...
    python -u scripts/postproc/split_stations.py --in "$LAGS" --out-dir "$DIR" \
        --prefix "$tag" --holdout "$HOLDOUT" --keep "$keep" --seed "$seed" || return 1
  ...
  for part in seen unseen; do
    ...
    [[ -f "$exp/eval_${part}/eval_per_lead_v2.json" ]] && \
      python -u scripts/postproc/record_run.py \
          ...
          --note "перенос: обучение на $keep станциях, жребий $seed, станции $part" 2>&1 | tail -1
  done
}
...
log "=== ALL DONE ==="
log "проверка у всех настроек одна и та же — те же 14 станций при данном жребии,"
log "  поэтому числа сравнимы напрямую. Растёт ли перенос с числом обучающих"
log "  станций и выходит ли на полку к 57 — это и есть ответ."
