...
# Нарезки и производные наборы кладём на /data: в /workdir квота 8 ГБ.
# mkdir обязателен — /data стирается при перезапуске виртуалки вместе с
# подкаталогами, и 30.08.2026 счёт описателей пропал именно из-за этого.
# Если /data недоступен, работаем рядом с корпусом.
DIR=/data/postproc
if ! mkdir -p "$DIR" 2>/dev/null || [[ ! -w "$DIR" ]]; then
  DIR="$REPO/data/postproc"; mkdir -p "$DIR" || { log "некуда писать — стоп"; exit 1; }
  log "/data/postproc недоступен, работаю в $DIR"
fi
