...
def main() -> None:
    ...
    # 10 МБ на лист — по факту загрузки для нашей области. Листы в 1 угловую
    # секунду (3601x3601), в сжатом виде от 4 МБ на равнине до 13 в горах.
    # Прежняя оценка в 3 МБ была от трёхсекундных листов и врала втрое.
    print(f"нужно листов: {len(tiles)} (примерно "
          f"{len(tiles) * 10 / 1024:.1f} ГБ в сжатом виде, "
          f"{len(tiles) * 25 / 1024:.1f} ГБ в распакованном)")
    print(" ".join(tiles))
