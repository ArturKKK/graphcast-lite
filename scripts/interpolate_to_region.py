    ...
    for p in range(P):
        ...
        if args.per_channel:
            for c in range(C):
                ...
                # Диагностика по статическим каналам (z_surf, lsm). Инерционный
                # прогноз воспроизводит их точно (base = 0), а у нас остаётся
                # ошибка — и это ОЖИДАЕМО: грубое поле 0.7° интерполируется на
                # сетку 0.25° и не содержит мелкого рельефа. Остаток равен как
                # раз тому, что даёт разрешение, и является содержательной
                # величиной, а не дефектом.
                #
                # Насторожить должна только большая ошибка (порядка разброса
                # самого поля) — тогда речь о рассогласовании развёртки узлов
                # или шкал, как было в двух исправленных багах этого скрипта.
                if vars_[c] in ("z_surf", "lsm"):
                    if cr > 0.5:
                        print(f"        ⚠️  {vars_[c]}: RMSE={cr:.4f} — слишком много даже "
                              f"для разницы разрешений; проверьте развёртку узлов и шкалы")
                    else:
                        print(f"        (i) {vars_[c]}: RMSE={cr:.4f} — остаток разрешения "
                              f"0.7° против 0.25°, так и должно быть")
