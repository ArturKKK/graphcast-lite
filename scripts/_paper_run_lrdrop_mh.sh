...
#
# Запуск: bash scripts/_paper_run_lrdrop_mh.sh   (сам уходит в фон)
set -uo pipefail
if [[ "${DAEMONIZED:-}" != "1" ]]; then
  DAEMONIZED=1 setsid nohup bash "$0" "$@" </dev/null >/dev/null 2>&1 &
  echo "запущено в фоне. лог: /workdir/paper_results/lrdrop_mh_master.log"; exit 0
fi
REPO=/workdir/graphcast-lite; VENV=/data/venvs/graphcast; OUT=/workdir/paper_results
D33R=/data/datasets/multires_krsk_33f
SRC=multires_krsk_33f; EXP=multires_krsk_33f_lrdrop_mh
mkdir -p "$OUT"; exec >>"$OUT/lrdrop_mh_master.log" 2>&1
cd "$REPO" || exit 1
...
if [[ ! -x "$VENV/bin/python" || ! -f "$D33R/data.npy" ]]; then
  ...
  bash scripts/_paper_setup_vm.sh >> "$OUT/lrdrop_mh_setup.log" 2>&1
  log "подготовка rc=$?"
fi
...
if [[ ! -f "$D33R/data.npy" ]]; then
  ...
  python -u scripts/build_multires_russia_33f.py --multires-dir /data/datasets/multires_krsk_19f_merge \
      ...
      --out-dir "$D33R" >> "$OUT/lrdrop_mh_build.log" 2>&1
  log "сборка rc=$?"
fi
...
python -u scripts/predict.py "experiments/$EXP" --data-dir "$D33R" --split test_only \
    ...
    --save-sample-metrics "$OUT/lrdrop_mh_roi_samples.npz" >> "$OUT/lrdrop_mh_roi.log" 2>&1
log "DONE оценка rc=$? | $(grep -oE 'skill=[-0-9.]+%' "$OUT/lrdrop_mh_roi.log" | tail -1)"
grep -E '^\s+(t2m|msl)\b' "$OUT/lrdrop_mh_roi.log" | tail -2
