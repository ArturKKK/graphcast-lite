...
#
# Место. Платформа убивает job примерно на 240 ГБ в /data, а цепочка в пиковый
# момент добавляет около 90 ГБ к занятому. Поэтому скрипт СНАЧАЛА считает баланс
...
LIMIT_GB=240
# Сколько ДОБАВИТСЯ к уже занятому в пиковый момент. Считано по шагам:
#   распаковка глобального   +82 (архив ещё на месте)
#   архив удалён             -74  → +8
#   сборка merge             +81  → +89   ← пик
#   SLIM: удалён wb2 19f     -82  → +7
#   сборка 33f               +60  → +67
# То есть максимум приращения — около 89 ГБ; берём 95 с запасом.
NEED_GB=95
...
log "в $DATA занято ${used} ГБ, лимит платформы ~${LIMIT_GB} ГБ, цепочке нужно ещё ~${NEED_GB} ГБ"
russia=$(du -sm "$DATA"/region_russia_* 2>/dev/null | awk '{s+=$1} END{print int(s/1024)}')
if (( used + NEED_GB > LIMIT_GB )); then
  log "не помещается: освободить не менее $((used + NEED_GB - LIMIT_GB)) ГБ"
