...
# ── 2.5. Опорное число: v3 на 12 шагов = +72 ч ────────────────────────
# Нужно для честного сравнения. Горизонт +24 ч, на котором мы меряли раньше, —
# худший для нашей же гипотезы: шестичасовой модели там хватает четырёх шагов,
# накапливать почти нечего. На +72 ч ей нужно двенадцать, а v5 — три. Вот там
# разница и должна проявиться, если она есть.
# Считаем ДО обучения: датасет уже собран, карта ещё свободна, займёт минут 20.
V3=experiments/wb2_512x256_33f_ar_v3/best_model.pth
if [[ -f "$V3" && ! -f "$OUT/v3_ar12.log" ]]; then
  log "START опорного прогона: v3, 12 шагов (+6…+72 ч)"
  python -u scripts/predict.py experiments/wb2_512x256_33f_ar_v3 --data-dir "$D33G" \
      --split test_only --ar-steps 12 --max-samples 200 --per-channel --no-save \
      --save-sample-metrics "$OUT/v3_ar12_samples.npz" \
      >> "$OUT/v3_ar12.log" 2>&1
  log "DONE v3 ar12 rc=$? | $(grep -oE 'skill=[-0-9.]+%' "$OUT/v3_ar12.log" | tail -1)"
  grep -E '^\s+\+(24|48|72)h' "$OUT/v3_ar12.log" | head -3
elif [[ ! -f "$V3" ]]; then
  log "ПРОПУСКАЮ опорный прогон: нет $V3"
  log "  веса надо добавить в репозиторий со старой машины: git add -f $V3"
else
  log "опорный прогон v3 уже есть, пропускаю"
fi
