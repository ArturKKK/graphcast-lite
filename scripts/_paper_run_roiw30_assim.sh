...
# Лог:    /workdir/paper_results/a30_master.log
#
...
if [[ ! -f "$D33/data.npy" ]]; then
  ...
  if [[ ! -f "$GEXTRA/coords.npz" ]]; then
    ...
    python -u scripts/fix_global_extra_coords.py --merge-dir "$MERGE" --extra-dir "$GEXTRA" >> "$OUT/a30_build.log" 2>&1 \
      || { log "восстановить оси не удалось — см. a30_build.log"; exit 1; }
  fi
  python -u scripts/build_multires_russia_33f.py \
      --multires-dir "$MERGE" --extra-dir "$GEXTRA" \
      --region-extra-dir "$REXTRA" --out-dir "$D33" \
      >> "$OUT/a30_build.log" 2>&1
  rc=$?
...
CKPT="$HEAVY/krsk_roiw30_assim.pth"
...
for L in 50000 100000 150000 200000; do
  for S in 0.3 0.5 1.0; do
    run "a30_val_oi_L${L}_s${S}" --split val --ar-steps 4 --max-samples $N_VAL --region $ROI \
...
run a30_test_baseline --split test_only --ar-steps 4 --max-samples $N_TEST --region $ROI
run a30_test_baseline_inner --split test_only --ar-steps 4 --max-samples $N_TEST --region $INNER

for A in 0.5 0.7 0.9; do
  run "a30_test_nudge_a${A}" --split test_only --ar-steps 4 --max-samples $N_TEST --region $ROI \
      --assim-method nudging --nudging-mode sequential --nudging-alpha $A \
      --obs-sparsity 0.1 --obs-roi-only --obs-seed 42
done

for SEED in 42 43 44; do
  run "a30_test_oi10_seed${SEED}" --split test_only --ar-steps 4 --max-samples $N_TEST --region $ROI \
      --assim-method oi --obs-sparsity 0.1 --obs-roi-only --obs-seed $SEED \
      --oi-corr-len 100000 --oi-sigma-o 0.5
done

run a30_test_oi1 --split test_only --ar-steps 4 --max-samples $N_TEST --region $ROI \
    --assim-method oi --obs-sparsity 0.01 --obs-roi-only --obs-seed 42 \
    --oi-corr-len 200000 --oi-sigma-o 0.5

# ── 5. Длинная развёртка: 14 суток ────────────────────────────────────
run a30_long_noda --split test_only --ar-steps $AR_LONG --max-samples $N_LONG --region $ROI

run a30_long_oi10 --split test_only --ar-steps $AR_LONG --max-samples $N_LONG --region $ROI \
    --assim-method oi --obs-sparsity 0.1 --obs-roi-only --obs-seed 42 \
    --oi-corr-len 100000 --oi-sigma-o 0.5

run a30_long_oi10_first4 --split test_only --ar-steps $AR_LONG --max-samples $N_LONG --region $ROI \
    --assim-method oi --obs-sparsity 0.1 --obs-roi-only --obs-seed 42 \
    --oi-corr-len 100000 --oi-sigma-o 0.5 --oi-first-k 4

run a30_long_oi1 --split test_only --ar-steps $AR_LONG --max-samples $N_LONG --region $ROI \
    --assim-method oi --obs-sparsity 0.01 --obs-roi-only --obs-seed 42 \
    --oi-corr-len 200000 --oi-sigma-o 0.5
