...
# Запуск: bash scripts/_paper_run_roiw30_assim.sh [модель] [bg|an]
#
# ВАЖНО про режим метрики (разобрались 25.08.2026). Усвоение применяется на
# каждом шаге развёртки, включая первый, и наблюдениями, действительными на тот
# же срок. Если писать в метрику поле ПОСЛЕ коррекции (режим an), получится
# АНАЛИЗ: оценка состояния, куда вошли наблюдения этого же срока, да ещё и
# проверенная по ERA5, из которой эти наблюдения и взяты. Прогнозом это называть
# нельзя. Режим bg (умолчание) пишет в метрику поле ДО коррекции, а вперёд
# по-прежнему уходит исправленное — меряется шестичасовой прогноз из
# предыдущего анализа.
#
# Подбор (L, sigma_o) идёт в том же режиме, и это существенно: на анализе
# оптимум смещается в сторону «верить наблюдениям сильнее», потому что так
# лучше ложится на них же. Для прогноза оптимум может быть другим.
# Лог:    /workdir/paper_results/a_${TAG}_master.log
#
...
# ПЕРЕСЧЁТ СЕРИИ УСВОЕНИЯ НА НОВОЙ ОСНОВНОЙ МОДЕЛИ (roiw30).
#
# Прежняя серия (a33_*) считалась на multires_krsk_33f: t2m 1,32/1,53/1,59/1,66 °C,
# агрегат по области 73,41 %. С 21.08 лучшая модель — multires_krsk_33f_roiw30
# (вес области 30 в лоссе): t2m 1,23/1,43/1,47/1,53 °C, агрегат 74,85 %.
# Числа усвоения в статье должны стоять на той же модели, что и числа прогноза,
# иначе таблицы несопоставимы между собой.
#
# Опорные значения прежней серии, с чем сравнивать:
#   без усвоения      +24 ч: 75,31 %,  t2m 1,32/1,53/1,59/1,66
#   нуджинг a=0,9     +24 ч: 76,96 %,  t2m 1,25/1,45/1,50/1,57
#   ОИ, 10 станций    +24 ч: 86,3-86,7 % по трём сидам, t2m 0,80/0,91/0,90/0,91
#   ОИ, 1 станция     +24 ч: 79,96 %,  t2m 1,13/1,32/1,33/1,37
#   14 суток без усвоения: 2,31 %; с ОИ каждые 10 шагов: 76,58 %
#
...
# Модель задаётся аргументом: какую выберем основной для статьи, на той и
# считаем усвоение. Логи и веса именуются по ней, так что серии не мешают
# друг другу.
EXP=${1:-multires_krsk_33f_roiw30}
ROI="50 60 83 98"
...
# или другой оценки — потерять и то, и другое.
...
if [[ ! -f "$D33/data.npy" ]]; then
  ...
  if [[ ! -f "$GEXTRA/coords.npz" ]]; then
    ...
    python -u scripts/fix_global_extra_coords.py --merge-dir "$MERGE" --extra-dir "$GEXTRA" >> "$OUT/a_${TAG}_build.log" 2>&1 \
      || { log "восстановить оси не удалось — см. a_${TAG}_build.log"; exit 1; }
  fi
  python -u scripts/build_multires_russia_33f.py \
      --multires-dir "$MERGE" --extra-dir "$GEXTRA" \
      --region-extra-dir "$REXTRA" --out-dir "$D33" \
      >> "$OUT/a_${TAG}_build.log" 2>&1
  rc=$?
...
CKPT="$HEAVY/krsk_${TAG}_assim.pth"
...
run a_${TAG}_test_baseline --split test_only --ar-steps 4 --max-samples $N_TEST --region $ROI
run a_${TAG}_test_baseline_inner --split test_only --ar-steps 4 --max-samples $N_TEST --region $INNER
