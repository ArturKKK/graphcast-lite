...
# Хотим предсказывать следующий шаг и некоторые переменные уточнять настоящими(усвоение)
# можно ли продифиренцировать резуоьтат работы модели по предыдущему шагу
# как я на инференсе беру граничные
# еще скинуть решение уравнения которое на скринах


# За время практики я разобрал ключевые статьи по нейросетевому прогнозу погоды (в т.ч. GraphCast в Science) и
# воспроизвел результаты инференса оригинальной модели. Учитывая её жёсткую архитектуру,
# сделал собственную 2.5D-модель с обучением и одно-/многошаговым прогнозом; 
# значения получаются в физических единицах и отображаются в погодных картах. Имеется визуализация графа. 
# Добавил инференс по региону и отдельный режим для Новосибирска — прогноз примерно на 20% точнее наивного «завтра = сегодня». 
# В среднем на горизонте до суток ошибка ниже примерно на треть, чем "наивного", и на +6 часов — почти на 36%;
# картина атмосферных аномалий хорошо совпадает с наблюдениями.

# план:
# - реализация алгоритма усвоения данных для графовой модели прогноза погоды

# Направление исследования:
# Краткосрочный прогноз погоды на графовых нейросетях в 2.5D-представлении с региональным инференсом.

# Цель:
# Создать лёгкую, воспроизводимую модель, которая стабильно превосходит наивную персистентность («завтра = сегодня»)
# глобально и по регионам, отдаёт значения в физических единицах и даёт наглядные карты.

# Задачи семестра:
# Изучение литературы по нейросетевому прогнозу (включая GraphCast в Science).
# Воспроизведение инференса оригинального GraphCast и анализ ограничений его стека.
# Проектирование и реализация собственной 2.5D-модели: обучение, одно- и многошаговый прогноз.
# Региональный инференс и отдельный режим для Новосибирска; сравнение с персистентностью.

# План на 7 семестр

# Направление исследования:
# Усвоение наблюдений на инференсе для 2.5D-графовой модели прогноза погоды.

# Планируемые к решению проблемы:
# Корректная обработка искусственных границ при региональном инференсе (нет внешнего контекста).

# Цели и задачи:
# 1) Реализовать алгоритм усвоения на инференсе: слияние прогноза со свежими наблюдениями.
# 2) Исследовать и выбрать метод обработки границ — зафиксировать рабочий вариант.
# 3) Провести пилот на примере Новосибирска: показать работу модели с усвоением и без, описать эффект и ограничения.